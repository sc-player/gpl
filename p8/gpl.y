...
std::stack<Statement_block*> stmtStk;
std::vector<string> checkAnimation;
...
%type<union_stmt> end_of_statement_block
%type<union_stmt> statement_block
%type<union_stmt> if_block
%type<union_int> keystroke
%type<union_int> print_statement
%type<union_op> geometric_operator
...
program:
    ...
    {
	for(int i=0; i<checkAnimation.size(); ++i){
		Error::error(Error::NO_BODY_PROVIDED_FOR_FORWARD, checkAnimation[i]);
	}
    }
    ;
...
initialization_block:
    ...
    {
	Event_manager::instance()->add_handler(Window::INITIALIZE, $2);
    }
    ;
...
animation_block:
    ...
    {
	 Symbol *s = Symbol_table::instance()->lookup(*$2);
	 if(!s){ 
		Error::error(Error::NO_FORWARD_FOR_ANIMATION_BLOCK, *$2);
	}
	 else if(s->get_animation_block_value()->taken==true) Error::error(Error::PREVIOUSLY_DEFINED_ANIMATION_BLOCK,  *$2);
	 else if($4)
	 {
		if(s->get_animation_block_value()->get_parameter_symbol()->get_name()!=*$4){
			Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD, *$2);
		}
		s->get_animation_block_value()->taken=true;
		stmtStk.push(s->get_animation_block_value());
		for(int i=0; i<checkAnimation.size(); ++i){
			if(checkAnimation[i]==s->get_name())
			{
				checkAnimation.erase(checkAnimation.begin()+i);
			}
		}
	}
    }
...
animation_parameter:
	...
	Symbol* s;
	$$=$2;
	if(Symbol_table::instance()->lookup(*$2)!=NULL){
...
check_animation_parameter:
    ...
    {
	Symbol* s = Symbol_table::instance()->lookup(*$2);
	if(!s || (s->get_game_object_value()->type()!="triangle"))
	{
		if(s) Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD, *$2);
		$$=NULL;
	}
	else $$=new string(s->get_name());
    }
    | T_PIXMAP T_ID
    {
	Symbol* s = Symbol_table::instance()->lookup(*$2);
	if(!s || !((s->get_game_object_value()->type())=="pixmap")) 
	{
		if(s) Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD, *$2);
		$$=NULL;
	}
    ...
    {
	Symbol* s = Symbol_table::instance()->lookup(*$2);
	if(!s || !((s->get_game_object_value()->type())=="circle"))
	{
		if(s) Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD, *$2);
		$$=NULL;
	}
    ...
    {
	Symbol* s = Symbol_table::instance()->lookup(*$2);
	if(!s || !((s->get_game_object_value()->type())=="rectangle"))
	{
		if(s) Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD, *$2);
		$$=NULL;
	}
    ...
    {
	Symbol* s = Symbol_table::instance()->lookup(*$2);
	if(!s || !((s->get_game_object_value()->type())=="textbox"))
	{
		if(s) Error::error(Error::ANIMATION_PARAM_DOES_NOT_MATCH_FORWARD, *$2);
		$$=NULL;
	}
...
on_block:
    ...
    {
	Event_manager::instance()->add_handler((Window::Keystroke)$2, $3);
    }
    ;
...
statement_block_creator:
    {
	stmtStk.push(new Statement_block());
    }
    ;
...
if_statement:
    ...
    {
	if(!($3->get_type()&INT)) Error::error(Error::INVALID_TYPE_FOR_IF_STMT_EXPRESSION);
	else stmtStk.top()->stateVec.push_back(new If_stmt($3, $5, NULL));
    }
    | T_IF T_LPAREN expression T_RPAREN if_block T_ELSE if_block
    {
	if(!($3->get_type()&INT)) Error::error(Error::INVALID_TYPE_FOR_IF_STMT_EXPRESSION);
	else stmtStk.top()->stateVec.push_back(new If_stmt($3, $5, $7));
    }
    ;
...
for_statement:
    ...
    {
	if(!($7->get_type()&INT)) Error::error(Error::INVALID_TYPE_FOR_FOR_STMT_EXPRESSION);
	else stmtStk.top()->stateVec.push_back(new For_stmt($5, $11, $13, $7));
    }
    ;
...
print_statement:
    ...
    {
	if(!(($3->get_type()&INT)||($3->get_type()&DOUBLE)||$3->get_type()&STRING)) Error::error(Error::INVALID_TYPE_FOR_PRINT_STMT_EXPRESSION);
	stmtStk.top()->stateVec.push_back(new Print_stmt($3, line_count));
    }
    ;
...
exit_statement:
    ...
    {
	if(!($3->get_type()&INT)) Error::error(Error::EXIT_STATUS_MUST_BE_AN_INTEGER, gpl_type_to_string($3->get_type()));
	stmtStk.top()->stateVec.push_back(new Exit_stmt($3, line_count));
    }
    ;
...
assign_statement:
	...
	if($1->get_type()&INT){
		...
		{
			stmtStk.top()->stateVec.push_back(new Assign_stmt($1->get_var(), $3, 0));
		}
	}
	...
	{
		if(!($3->get_type()&ANIMATION_BLOCK)) Error::error(Error::ASSIGNMENT_TYPE_ERROR, gpl_type_to_string($1->get_type()), gpl_type_to_string($3->get_type()));
		else if($3->eval_animation_block()->get_parameter_symbol()->get_game_object_value()->type()!=$1->get_var()->sym->get_game_object_value()->type()) Error::error(Error::ANIMATION_BLOCK_ASSIGNMENT_PARAMETER_TYPE_ERROR, $1->get_var()->sym->get_game_object_value()->type(), $3->eval_animation_block()->get_parameter_symbol()->get_game_object_value()->type());
		else stmtStk.top()->stateVec.push_back(new Assign_stmt($1->get_var(), $3, 0));
	}
	else
	{
		std::string type =($1->get_type()&GAME_OBJECT)?($1->get_var()->sym->get_game_object_value()->type()):(gpl_type_to_string($1->get_type()));
		Error::error(Error::INVALID_LHS_OF_ASSIGNMENT, $1->get_name(), type);
	}
    }
	...
	if($1->get_type()&INT){
		if(!($3->get_type()&INT)){
			...
			stmtStk.top()->stateVec.push_back(new Assign_stmt($1->get_var(), $3, 1));
		}
	}
	else if($1->get_type()&DOUBLE){
		if(!(($3->get_type()&INT)||($3->get_type()&DOUBLE))){
		...
		std::string type =($1->get_type()&GAME_OBJECT)?($1->get_var()->sym->get_game_object_value()->type()):(gpl_type_to_string($1->get_type()));
		Error::error(Error::INVALID_LHS_OF_PLUS_ASSIGNMENT, $1->get_name(), type);
	}
    }
	...
	if($1->get_type()&INT){
		if(!($3->get_type()&INT)){
			...
			stmtStk.top()->stateVec.push_back(new Assign_stmt($1->get_var(), $3, 2));
		}
	}
	else if($1->get_type()&DOUBLE){
		if(!(($3->get_type()&INT)||($3->get_type()&DOUBLE))){
		...
		std::string type =($1->get_type()&GAME_OBJECT)?($1->get_var()->sym->get_game_object_value()->type()):(gpl_type_to_string($1->get_type()));
		Error::error(Error::INVALID_LHS_OF_MINUS_ASSIGNMENT, $1->get_name(), type);
	}
    }
    ;
...
expression:
	...
	if((($1->get_type()&INT)||($1->get_type()&DOUBLE))&&(($3->get_type()&INT)||($3->get_type()&DOUBLE))){
	...
	if((($1->get_type()&INT)||($1->get_type()&DOUBLE)||($1->get_type()&STRING))&&(($3->get_type()&INT)||($3->get_type()&DOUBLE)||($3->get_type()&STRING))){ 
	...
	if(!($1->get_type()&GAME_OBJECT)){
		Error::error(Error::OPERAND_MUST_BE_A_GAME_OBJECT, $1->get_var()->sym->get_name());
	}
	if(!($3->get_type()&GAME_OBJECT)){
		Error::error(Error::OPERAND_MUST_BE_A_GAME_OBJECT, $3->get_var()->sym->get_name());	
	}
