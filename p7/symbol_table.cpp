...
bool lessThan (Symbol* i,Symbol* j) { return (i->get_name()<j->get_name()); }
...
void Symbol_table::addSymbol(Symbol * sym){
	if(lookup(sym->get_name())==NULL){
		sTable.insert({sym->get_name(), sym});
		return;
	}
	else{
		Error::error(Error::PREVIOUSLY_DECLARED_VARIABLE, sym->get_name());
	}
}